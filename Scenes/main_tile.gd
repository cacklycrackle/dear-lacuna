extends Node2D
@onready var space_state = get_world_2d().direct_space_state
@onready var left_query = PhysicsPointQueryParameters2D.new()
@onready var right_query = PhysicsPointQueryParameters2D.new()
@onready var top_query = PhysicsPointQueryParameters2D.new()
@onready var bottom_query = PhysicsPointQueryParameters2D.new()
var moveable = false
var cursor
var axis = "m"
var ds = preload("res://Scenes/debug_sprite.tscn")
func _ready() -> void:
	
	left_query.position = Vector2(global_position)
	left_query.position.x -= 60
	left_query.collision_mask = 4
	left_query.collide_with_areas = true
	var a = ds.instantiate()

	right_query.position = Vector2(global_position)
	right_query.position.x += 60
	right_query.collision_mask = 4
	right_query.collide_with_areas = true
	
	top_query.position = Vector2(global_position)
	top_query.position.y -= 60
	top_query.collision_mask = 4
	top_query.collide_with_areas = true
	
	bottom_query.position = Vector2(global_position)
	bottom_query.position.y += 60
	bottom_query.collision_mask = 4
	bottom_query.collide_with_areas = true

func _input(event: InputEvent) -> void:
	if event.is_action_pressed("up") and moveable and Input.is_action_pressed("interact"):
		cursor.moveable = false
		if space_state.intersect_point(top_query).size() == 0:
			global_position.y -= 60
			top_query.position.y -= 60
			bottom_query.position.y -= 60
			left_query.position.y -= 60
			right_query.position.y -= 60
			cursor.global_position.y -= 60
	if event.is_action_pressed("down") and moveable and Input.is_action_pressed("interact"):
		cursor.moveable = false
		if space_state.intersect_point(bottom_query).size() == 0:
			global_position.y += 60
			top_query.position.y += 60
			bottom_query.position.y += 60
			left_query.position.y += 60
			right_query.position.y += 60
			cursor.global_position.y += 60
	if event.is_action_pressed("left") and moveable and Input.is_action_pressed("interact"):
		cursor.moveable = false
		if space_state.intersect_point(left_query).size() == 0:
			global_position.x -= 60
			left_query.position.x -= 60
			right_query.position.x -= 60
			top_query.position.x -= 60
			bottom_query.position.x -= 60
			cursor.global_position.x -= 60
	if event.is_action_pressed("right") and moveable and Input.is_action_pressed("interact"):
		cursor.moveable = false
		if space_state.intersect_point(right_query).size() == 0:
			global_position.x += 60
			left_query.position.x += 60
			right_query.position.x += 60
			top_query.position.x += 60
			bottom_query.position.x += 60
			cursor.global_position.x += 60
	if event.is_action_released("interact"):
		if cursor:
			cursor.moveable = true


func _on_area_2d_area_entered(area: Area2D) -> void:
	cursor = area.get_parent()
	moveable = true


func _on_area_2d_area_exited(area: Area2D) -> void:
	moveable = false
